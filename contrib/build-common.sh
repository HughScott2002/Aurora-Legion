...
# Shared build steps for the release artifact scripts. Sourced by
# build-tarball.sh and build-appimage.sh; not executable on its own.
#
# Everything assumes an Ubuntu 24.04 environment (see the guard), which
# is the oldest supported baseline (glibc 2.39, GTK 4.14, adw 1.5).
